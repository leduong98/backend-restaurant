...
class JwtServerAuthenticationConverter implements ServerAuthenticationConverter {
    ...
    public Mono<Authentication> convert(ServerWebExchange exchange) {
        Mono<String> tokenFromHeader = Mono.justOrEmpty(exchange.getRequest().getHeaders().getFirst(HttpHeaders.AUTHORIZATION))
                .filter(header -> header.startsWith(BEARER))
                .map(header -> header.substring(BEARER.length()));

        Mono<String> tokenFromCookie = Mono.justOrEmpty(exchange.getRequest().getCookies().getFirst("auth_token"))
                .map(HttpCookie::getValue).log("shhsshhsshshhs");
        return tokenFromHeader
                .switchIfEmpty(tokenFromCookie)
                .flatMap(token -> {
                    try {
                        UserDetails userDetails = createUserDetails(token);
                        return Mono.just(new JwtToken(token, userDetails));
                    } catch (Exception e) {
                        return Mono.empty();
                    }
                });
